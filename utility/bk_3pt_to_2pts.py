...
if len(sys.argv) != 5:
  print("Usage: ./bk_3pt_to_2pts.py <data_stem> <traj_start> <traj_end> <traj_inc>")
  exit(0)
...
for traj in xrange(traj_start, traj_end+1, traj_incr):
  ...
  for sep in seps:
    ...
    for t in xrange(0,data_this_sep.shape[0]):
      line = "{0:3d} {1:1.10e} {2:1.10e} {3:1.10e} {4:1.10e}".format(  \
          int(data_this_sep[t,1]), data_this_sep[t,2], \
          data_this_sep[t,3], data_this_sep[t,4], data_this_sep[t,5])
      print(line, file=f)
    f.close()
