...
###################################################################
## Plot midpoint of BK ratio as a function of pion separation
##
...
#     else:
#       idx = int((sep-1)/2)
#       BK[sep,1] = 0.5*( tmp[idx,1] + tmp[idx+1,1] )
#       BK[sep,2] = 0.5*( tmp[idx,2] + tmp[idx+1,2] )
BK = np.genfromtxt(data_stem)

# Plot data
if not np.isnan(t_fit[0]):
  idx_ti = np.where(BK[:,0] == t_fit[0])[0][0]
  if BK[-1,0] >= t_fit[1]:
    idx_tf = np.where(BK[:,0] == t_fit[1])[0][0] + 1
  elif BK[-1,0] == t_fit[1]-1:
    t_fit[1] -= 1
  else:
    print("Dimensions of data do not match -tfit option.")
    exit(-1)
  if data_label:
    plt.errorbar(BK[0:idx_ti,0], BK[0:idx_ti,1], yerr=BK[0:idx_ti,2], marker='o', color='k', ls='none', label=data_label)
  else:
    plt.errorbar(BK[0:idx_ti,0], BK[0:idx_ti,1], yerr=BK[0:idx_ti,2], marker='o', color='k', ls='none')
  plt.errorbar(BK[idx_ti:idx_tf,0], BK[idx_ti:idx_tf,1], yerr=BK[idx_ti:idx_tf,2], marker='o', color='r', ls='none')
  plt.errorbar(BK[idx_tf:,0], BK[idx_tf:,1], yerr=BK[idx_tf:,2], marker='o', color='k', ls='none')
else:
  if data_label:
    plt.errorbar(BK[:,0], BK[:,1], yerr=BK[:,2], marker='o', color='k', ls='none', label=data_label)
  else:
    plt.errorbar(BK[:,0], BK[:,1], yerr=BK[:,2], marker='o', color='k', ls='none')
