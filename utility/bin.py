...
if len(sys.argv) != 6:
  print("Usage: ./bin.py <data_stem> <traj_start> <traj_end> <traj_inc> <bin_size>")
  exit(0)
...
for traj in range(traj_start, traj_end+1, bin_size*traj_incr):
  ...
  data_binned[:,1:,idx] = np.average(data[:,1:,idx*bin_size:(idx+1)*bin_size], axis=2)
  fout = data_stem + ".bin_" + str(bin_size) + "." + str(traj)
  f = open(fout,'w')
  for row in xrange(0,Nrows):
    line = "{0:3d}".format(int(data_binned[row,0,idx]))
