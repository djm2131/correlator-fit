...
double Fitter::chisq(const gsl_vector* x, void* data)
{
  double* t = ((fit_data*)data)->t;
  double* C = ((fit_data*)data)->C;
  double* w = ((fit_data*)data)->w;
...
{
  return static_cast<fit_data*>((fit_data*)data)->me->chisq(x,data);
}
...
double Fitter::LM_fit(int jknife_idx, fit_data& fd, fit_results& fr)
{
...
double Fitter::NM_fit(int jknife_idx, fit_data& fd, fit_results& fr)
{
  // Initialize GSL objects
